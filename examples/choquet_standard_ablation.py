# Choquet SAC with Standard Discounting - Ablation Study  
# This version uses Choquet integral with standard discounting (shortened baseline)
...
# Import behavioral utilities for Choquet integral only
from behavioral.distortions import PrelecDistortion, choquet_expectation


@dataclass
class Args:
    exp_name: str = "choquet_standard_ablation"
    ...
    wandb_project_name: str = "ablation-choquet-standard"
    ...
    gamma: float = 0.99
    """the discount factor gamma (standard discounting)"""
    tau: float = 0.005
    ...
    # Choquet integral configuration
    ...
    prelec_alpha: float = 0.65
    """Prelec distortion parameter alpha - moderate risk aversion"""
    prelec_eta: float = 0.4
    """Prelec distortion parameter eta - moderate curvature"""
    
    # Standard discounting (no hyperbolic)
    discounting_mode: str = "standard"
    """discounting mode: standard for this ablation"""
...
if __name__ == "__main__":
    ...
    # Initialize Prelec distortion function only
    prelec_distortion = PrelecDistortion(alpha=args.prelec_alpha, eta=args.prelec_eta)
    
    print(f"Choquet + Standard Discounting Ablation Experiment")
    print(f"Using Choquet integral: {args.use_choquet}")
    print(f"Using standard discounting: {args.discounting_mode}")
    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step > args.learning_starts:
            ...
            with torch.no_grad():
                # Choquet + Standard discounting combination
                batch_size = data.next_observations.size(0)
                
                # Sample multiple actions for Choquet expectation
                ...
                soft_q_values = min_qf_next - alpha * next_log_probs
                
                # Apply Choquet expectation
                ...
                # Use standard discounting with Choquet expectation
                next_q_value = data.rewards.flatten() + (1 - data.dones.flatten()) * args.gamma * choquet_expectation_values
            ...
            if global_step % 100 == 0:
                ...
                writer.add_scalar("charts/effective_gamma", args.gamma, global_step)
                ...
                # Standard discounting tracking
                writer.add_scalar("discounting/effective_gamma", args.gamma, global_step)
                writer.add_scalar("discounting/mode", 0.0, global_step)  # 0.0 for standard
